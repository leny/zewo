...
final class Table extends \Zewo\Tools\Cached {
	...
	private function _buildTableInfos() {
		$sTableInfosQuery = "SELECT DISTINCT COLUMNS.COLUMN_NAME, COLUMNS.COLUMN_DEFAULT, COLUMNS.IS_NULLABLE, COLUMNS.COLUMN_TYPE, PRI.CONSTRAINT_NAME AS 'PRIMARY', REF.REFERENCED_TABLE_NAME, REF.REFERENCED_COLUMN_NAME FROM COLUMNS LEFT JOIN KEY_COLUMN_USAGE PRI ON(COLUMNS.COLUMN_NAME = PRI.COLUMN_NAME AND PRI.TABLE_NAME = COLUMNS.TABLE_NAME AND PRI.TABLE_SCHEMA = COLUMNS.TABLE_SCHEMA AND PRI.CONSTRAINT_NAME = 'PRIMARY') LEFT JOIN KEY_COLUMN_USAGE REF ON(COLUMNS.COLUMN_NAME = REF.COLUMN_NAME AND REF.TABLE_NAME = COLUMNS.TABLE_NAME AND REF.TABLE_SCHEMA = COLUMNS.TABLE_SCHEMA AND REF.CONSTRAINT_NAME <> 'PRIMARY') WHERE COLUMNS.TABLE_SCHEMA = '" . $this->_sBase . "' AND COLUMNS.TABLE_NAME = '" . $this->_sTable . "' ORDER BY COLUMNS.ORDINAL_POSITION ASC";
		...
		foreach( $aTableInfos as $aColumnInfo ) {
			...
			if( $aColumnInfo['PRIMARY'] == 'PRIMARY' ) {
